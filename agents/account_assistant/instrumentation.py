...
wrapping the real cause, which auto-instrumentation has no reason to
unwrap. The result: the trace shows a generic tool-call error rather than
the actual denial/failure reason.
...
def _get_tracer():
    ...
    if _tracer is None:
        ...
        _tracer = trace.get_tracer("account-assistant-agent.mcp-tool-call")
    return _tracer
